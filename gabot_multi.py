...
import requests # <--- BARIS BARU UNTUK KONEKSI HTTP POST KE MAKE.COM
import qrcode # <--- Ditambahkan agar fungsi generate_qr berfungsi
from io import BytesIO # <--- Ditambahkan agar fungsi generate_qr berfungsi
...
# === ROUTE BARU UNTUK MENGIRIM ORDER KE MAKE.COM ===
# Route ini dipanggil oleh frontend/chat setelah order divalidasi
@app.route("/submit_order", methods=["POST"])
def submit_order():
    if "client_id" not in session:
        return jsonify({"status": "error", "message": "Unauthorized"}), 401

    # 1. Ambil data order dari frontend/request
    data = request.get_json()
    client_id = session.get("client_id")
    client = Client.query.get(client_id)

    # Siapkan payload untuk dikirim ke Make.com
    payload = {
        "client_slug": client.slug,
        "client_name": client.business_name,
        "wa_number": client.whatsapp,
        # Asumsi data berikut dikumpulkan di frontend/chat.js (Anda harus memastikan variabelnya ada di 'data')
        "product_type": data.get("product_type"), 
        "qty_final": data.get("qty_final"),
        "order_specs": data.get("order_specs", "N/A") # Spesifikasi tambahan
    }

    # 2. Kirim POST Request ke Make.com Webhook
    try:
        response = requests.post(WEBHOOK_MAKE_COM_URL, json=payload, timeout=10)

        if response.status_code == 200:
            return jsonify({
                "status": "success",
                "message": "Order berhasil dikirim ke SSOT (Airtable via Make.com)."
            }), 200
        else:
            # Jika Make.com menolak request (misal error mapping)
            return jsonify({
                "status": "warning", 
                "message": "Data diterima, namun Make.com merespon error.",
                "make_response": response.text
            }), 202 

    except requests.exceptions.RequestException as e:
        # Jika terjadi error koneksi/timeout
        print(f"WEBHOOK ERROR: {e}")
        return jsonify({
            "status": "error", 
            "message": "Gagal terhubung ke server Make.com (SSOT). Harap hubungi admin."
        }), 500
...
def chat():
    # PERHATIAN: Route ini HANYA UNTUK FAQ TF-IDF, BUKAN ORDER!
    if "client_id" not in session:
...
@app.before_request
def check_approval():
    allowed = ["login", "register", "static", "logout", "chat", "public_chat", "healthz", "submit_order"] # submit_order ditambahkan
    ...
    with app.app_context():
        ...
        if not Client.query.filter_by(username="admin").first():
            admin = Client(
                ...
                role=RoleEnum.admin, # Menggunakan Enum
                business_name="Admin",
                whatsapp="0000000000",
